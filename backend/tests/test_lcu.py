...
def test_build_champ_select_snapshot_strips_enemy_names_and_maps_bans():
    ...
    assert (local.cell_id, local.champion_id, local.champion, local.name, local.state) == (
        2, 1, "Annie", "FixturePlayer24", "picked",
    )
    ...
    assert "FixturePlayer14" not in json.dumps(snapshot.model_dump())
...
def test_build_ingame_snapshot_maps_players_scores_items_events():
    ...
    assert snapshot.local_summoner == "FixturePlayer24"
    ...
    me = next(p for p in order if p.summoner == "FixturePlayer24")
    ...
    assert (kill.actor, kill.victim) == ("FixturePlayer24", "FixturePlayer10")
...
async def test_idle_then_champ_select_then_in_game_transitions():
    ...
    assert any(c["is_local"] and c["name"] == "FixturePlayer24" for c in snap["ally"])
    assert "FixturePlayer14" not in json.dumps(snap)
...
async def test_in_game_window_polls_allgamedata_and_publishes_rich_state():
    ...
    assert live_state["local_summoner"] == "FixturePlayer24"
