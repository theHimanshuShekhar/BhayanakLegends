...
import { ItemSpikeCard } from "../components/champions/ItemSpikeCard";
...
export function ChampionsPage() {
  ...
  const activeRole = selected ?? (roles.includes("MIDDLE") ? "MIDDLE" : (roles[0] ?? null));

  const headerEntry = useMemo(
    () => pack.data?.tier_list.find((t) => t.role === activeRole) ?? null,
  ...
  const matchups = useMemo(
    () => (pack.data?.matchup_examples ?? []).filter((m) => m.role === activeRole),
    [pack.data, activeRole],
  );
      ...
      {pack.data && activeRole && (
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
            ...
            <MatchupsCard matchups={matchups} />
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
            {compFindings.length > 0 && <CompCard findings={compFindings} />}
            {damageFit && <DamageFitCard finding={damageFit} />}
            <RoleTierList role={activeRole} rows={tierRows} trapPicks={trapPicks} />
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
            <BuildOrderCard />
            <ItemSpikeCard points={trajectories.data ?? []} />
