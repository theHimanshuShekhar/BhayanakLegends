...
are generated by `backend/tools/build_pack.py`. The producer requires an
explicit `--feature-store` input for numeric-bearing output or `--bootstrap`
for the deterministic issue-backed seed. It records the Feature Store
manifest hash and generator revision in every numeric-bearing provenance
block. The active directory is populated atomically from the bundled seed on
first startup; an existing active pack wins over a changed bundled seed.
...
The bundled issue-backed seed describes 125,031 eligible matches and
1,250,310 participant performances across patches 14.17–16.17. Its
Surrender Advisor is withheld because the observed 22.7 percentage-point
gap exceeds the five-point tolerance. Baron comeback lift is omitted.
Route archetypes are approximate diagnostics, not advice; build evidence is
withheld and contains no raw sequence rows. A number is emitted only when
the canonical producer has the corresponding exact evidence.
