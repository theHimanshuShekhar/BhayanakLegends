// Sidecar lifecycle and process ownership.
...
pub(crate) struct SidecarStateInner {
    pub(crate) handle: Option<SidecarInfo>,
    pub(crate) startup_error: Option<SidecarStartupError>,
    pub(crate) lifecycle: SidecarLifecycle,
}
...
struct Supervisor<A: SidecarProcessAdapter> {
    adapter: A,
...
impl<A: SidecarProcessAdapter> Supervisor<A> {
    #[allow(dead_code)]
    ...
    for attempt in 1..=MAX_LAUNCHES {
        ...
        match supervisor.launch(token.clone()) {
            ...
            Err(error) => {
                ...
                eprintln!("sidecar launch attempt {attempt}/{MAX_LAUNCHES} failed: {last_error}");
                if attempt < MAX_LAUNCHES
                    && !sleep_with_shutdown(retry_backoff, &supervisor.shutdown)
                {
...
mod tests {
    ...
    fn lifecycle_publishes_credentials_only_after_authenticated_health() {
        ...
        let mut supervisor = Supervisor::new(adapter);
    ...
    fn shutdown_wins_race_with_unexpected_exit_without_recovery() {
        ...
        let shutdown = ShutdownToken::default();
        let mut supervisor = Supervisor::with_shutdown(adapter, shutdown.clone());
        supervisor.launch("token-a".into()).unwrap();
        shutdown.request();
